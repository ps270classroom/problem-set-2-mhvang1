...
num_candidates <- n_distinct(fb_ads$cand_name)
incumbency_table <- fb_ads |>
    group_by(incumbency) |>
    summarise (num_candidates = n_distinct(cand_name))
...
plot_q3 <- fb_ads |>
  ...
  geom_histogram(n = 15, fill = "skyblue", color = "black") +
...
print(plot_q3)
```
There were less than 10 million ad impressions on Facebook.
\newpage
...
fb_top_plot <- ggplot(top_cand, aes(x = impressions_millions, y = reorder(cand_name_last, impressions_millions), fill = party)) +
