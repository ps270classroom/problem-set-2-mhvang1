...
## Answer 1
fb_ads <- read_csv()
num_candidates <- n_distinct(fb_ads$candidate_name)
incumbency_table <- fb_ads %>%
    group_by(incumbency_status) %>%
    summarise (num_candidates = n_distinct(candidates_name))
...
plot_q3 <- fb_ads %>%
  filter(race_type == "US House") %>%
  ggplot(aes(x = impressions_millions)) + 
  geom_histogram(binwidth = 1, fill = "skyblue", color = "black") +
  labs(title = "Distribution of Facebook Ad Impressions (in Millions) for US House Races", x = "Ad Impressions (in Millions)", y = "Frequency")
print(plot_q3)
...
## Answer 4
fb_ads <- 
  mutate(cand_name_last = sub(".*\\s", "", candidate_name))
...
fb_top_plot <- ggplot(top_house_candidates, aes(x = impressions_millions, y = reorder(cand_name_last, impressions_millions), fill = party)) +
  geom_bar(stat = "identity") +
  labs(title = "Top 15 US House Candidates by Facebook Ad Impressions",
       x = "Ad Impressions (in Millions)",
